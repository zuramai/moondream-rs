...
impl Moondream {
    pub fn new(client: TritonGRPCClient, tokenizer: Tokenizer, config: Config) -> Self {
        Self {
            client,
            tokenizer,
            config,
    ...
    pub async fn encode_image(&self, image: Array4<f32>) -> Result<ArrayD<f32>> {
        let mut inputs = InferInputs::new_f16();
        inputs.add_input_fp16("input", image.into_dyn());

        let mut outputs = self.client
            .infer_http("vision_encoder".to_string(), "1".to_string(), inputs, vec!["output"])
            .await?;

        let output = outputs.remove("output").ok_or(Error::new("Missing output"))?;
    ...
    async fn run(&self, image: ArrayD<f32>, object: &str) -> Result<DetectOutput> {
